...
def main():
	...
	res3 = open(outputDir + 'BioXpress_de','w')
	res4 = open(outputDir + 'BioXpress_cancer.txt','w')
			...
			for row in csvreader:
				if not row[0] == "GeneID" and not row[0] == "miRNAID":
					...
					for oneCan in range(0, len(doidCancer)):
						...
						if oneCan == len(doidCancer) - 1:
							doidDic[doId + '\t' + parentDoid + '\t' + doTerm + '\t' + ' '.join(row[-2].strip().split()) + '\n'] = 1
						else:
							doidDic[doId + '\t' + parentDoid + '\t' + doTerm + '\t-\n'] = 1
					platform = row[11] if row[11] in ["Manual","TextMining"] else "TCGA"
					cancerDic['-\t' + tcgaCancer.strip() +'\t'+' '.join(row[-2].strip().split())+ '\t' + platform + '\n'] = 1
					if row[11] == "Manual":
						sig = "Yes"
					elif row[-5] == "NA":
						sig = "No"
		...
		with open(fileList, 'rb') as csvfile:
			...
			for row in csvreader:
				if row[0] != "geneName" and row[0] != "miRNAName":
					cancerType = row[1]
					platform = row[2] if row[2] in ["Manual","TextMining"] else "TCGA"
					cancerDic['-\t' + cancerType +'\t'+referCancer[cancerType]+'\t'+platform+'\n'] = 1
					doidCancer = referCancer[cancerType].split(' & ')
					for oneCan in range(0, len(doidCancer)):
						doId = doidCancer[oneCan].split()[0].split(":")[-1].strip()
						if not doId.isdigit():
							doId = str(0)
						...
						parentDoid = cancerName[0].split(':')[-1].strip()
						if oneCan == len(doidCancer) - 1:
							doidDic[doId + '\t' + parentDoid + '\t' + doTerm + '\t'+ referCancer[cancerType] + '\n'] = 1
						else:
							 doidDic[doId + '\t' + parentDoid + '\t' + doTerm + '\t-\n'] = 1
