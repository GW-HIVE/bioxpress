...
def main():
    ...
    sbj_count = {}
    file_list = glob.glob("/data/projects/bioxpress/v-5.0/generated/deseq/per_tissue/*/results_significance.csv")
    i = 0
    for in_file in file_list:
        i += 1
        if i%10 == 0:
            print "%s of %s per_tissue files processed" % (i, len(file_list))
    ...
    FW1 = open("/data/projects/bioxpress/v-5.0/generated/publisher/bioxpress-final.de.csv", "w")
    FW1.write("\"%s\"\n" % ("\",\"".join(newrow) )) 

    for study_id in do_dict:
        in_file = "/data/projects/bioxpress/v-5.0/generated/deseq/per_study/%s/results_significance.csv" % (study_id)
        data_frame = {}
        csvutil.load_sheet(data_frame,in_file, ",")
        f_list = data_frame["fields"]
        for row in data_frame["data"]:
